export function createLinksLayerManager(d3) {
	var linksLayer = null;
	...
	function updateLinks(params) {
		...
		if (x && typeof x.copy === "function") {
			xForLinks = x.copy();
			if (typeof xForLinks.clamp === "function") {
				xForLinks.clamp(false);
			}
		}
		...
		for (var i = 0; i < tasks.length - 1; i++) {
			...
			var currId = curr && curr.__uid ? String(curr.__uid) : ("task-" + i);
			var nextId = next && next.__uid ? String(next.__uid) : ("task-" + (i + 1));

			lineData.push({
				id: currId + "->" + nextId,
				x1: xForLinks(curr.endDate),
				y1: rowCurr.y + rowCurr.height / 2,
				x2: xForLinks(next.startDate),
				y2: rowNext.y + rowNext.height / 2
			});
		}

		var links = linksGroup.selectAll(".link-path").data(lineData, function(d) {
			return d.id;
		});
		links.exit().remove();
